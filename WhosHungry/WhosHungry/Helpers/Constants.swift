//
//  Constants.swift
//  WhosHungry
//
//  Created by Kelsey Sparkman on 9/14/20.
//  Copyright © 2020 Kelsey Sparkman. All rights reserved.
//

import Foundation

struct Constants {
    
    static let userContainer = "userContainer"
    static let gameContainer = "gameContainer"
    static let inviteCode = "inviteCode"
    static let firstName = "firstName"
    static let lastName = "lastName"
    static let user = "user"
    static let address = "formatted_address"
    static let users = "users"
    static let city = "city"
    static let radius = "radius"
    static let mealType = "mealType"
    static let uid = "uid"
    static let email = "email"
    static let creatorID = "creatorID"
    static let yelpAPIKey = "yelpAPIKey"
    static let googleAPIKey = "googleAPIKey"
    static let submittedVotes = "submittedVotes"
    static let usersVotes = "usersVotes"
    static let gameHasBegun = "gameHasBegun"
    static let displayedRestaurantDefaults = "likedRestaurants"
    static let allVotesSubmitted = "allVotesSubmitted"
    static let winningRestaurant = "winningRestaurant"
    static let numberOfRevotes = "numberOfRevotes"
    static let onResultsPage = "onResultsPage"
    static let noWinningRestaurant = "noWinningRestaurant"
}
